...
const monthly = (
  dayOfMonth: number,
  overrides: Partial<RecurrenceSchedule> = {},
): RecurrenceSchedule => ({
  frequency: "MONTHLY",
  interval: 1,
...
describe("occurrencesInPeriod — MONTHLY", () => {
  ...
  it("kotwica 31. w lutym roku zwykłego → 28.02", () => {
    expect(occurrencesInPeriod(monthly(31), period("2026-02-01", "2026-02-28"))).toEqual([
      "2026-02-28",
    ]);
  });

  it("kotwica 31. w lutym roku przestępnego → 29.02", () => {
    expect(occurrencesInPeriod(monthly(31), period("2028-02-01", "2028-02-29"))).toEqual([
      "2028-02-29",
    ]);
  });
...
describe("occurrencesInPeriod — YEARLY", () => {
  ...
  it("raz w roku, w miesiącu startDate", () => {
    ...
    expect(occurrencesInPeriod(insurance, period("2026-03-01", "2026-03-31"))).toEqual([
      "2026-03-15",
    ]);
  ...
  it("kotwica 29.02 w roku zwykłym → 28.02", () => {
    ...
    expect(occurrencesInPeriod(leapAnchor, period("2026-02-01", "2026-02-28"))).toEqual([
      "2026-02-28",
    ]);
    expect(occurrencesInPeriod(leapAnchor, period("2028-02-01", "2028-02-29"))).toEqual([
      "2028-02-29",
    ]);
  });
});
