...
describe('assembleBudgetInput — periodIncome', () => {
  ...
  it('REGULAR z potwierdzonym wpływem → faktyczna kwota zamiast oczekiwanej', () => {
    const input = assembleBudgetInput(
      snapshot({
        ...
        incomeEntries: [
          { incomeSourceId: 'salary', amount: g(760_000), date: d('2026-09-10'), status: 'CONFIRMED' },
        ],
      }),
    );
  ...
  it('REGULAR co 2 tygodnie: jedna wypłata potwierdzona, druga jeszcze oczekiwana', () => {
    const input = assembleBudgetInput(
      snapshot({
        ...
        incomeEntries: [
          { incomeSourceId: 'salary', amount: g(310_000), date: d('2026-09-04'), status: 'CONFIRMED' },
        ],
      }),
    );
  ...
  it('więcej potwierdzonych wpływów niż wystąpień (np. premia) → liczy się faktyczna suma', () => {
    const input = assembleBudgetInput(
      snapshot({
        incomeSources: [salary()],
        incomeEntries: [
          { incomeSourceId: 'salary', amount: g(800_000), date: d('2026-09-10'), status: 'CONFIRMED' },
          { incomeSourceId: 'salary', amount: g(200_000), date: d('2026-09-20'), status: 'CONFIRMED' },
        ],
      }),
    );
  ...
  it('IRREGULAR: tylko potwierdzone wpływy z okresu', () => {
    const input = assembleBudgetInput(
      snapshot({
        incomeSources: [{ id: 'gigs', kind: 'IRREGULAR', expectedAmount: null, isActive: true, schedule: null }],
        incomeEntries: [
          { incomeSourceId: 'gigs', amount: g(120_000), date: d('2026-09-05'), status: 'CONFIRMED' },
          ...
          { incomeSourceId: 'gigs', amount: g(999_000), date: d('2026-08-31'), status: 'CONFIRMED' },
        ],
      }),
    );
  ...
  it('zarchiwizowane źródło: bez oczekiwanej kwoty, ale potwierdzony wpływ się liczy', () => {
    const input = assembleBudgetInput(
      snapshot({
        ...
        incomeEntries: [
          { incomeSourceId: 'salary', amount: g(400_000), date: d('2026-09-02'), status: 'CONFIRMED' },
        ],
      }),
    );
...
describe('assembleBudgetInput — remainingFixedCommitments', () => {
  ...
  it('opłacony czynsz (potwierdzona transakcja z tą regułą) znika z listy', () => {
    const input = assembleBudgetInput(
      snapshot({
        expenseRules: [rent()],
        transactions: [
          { amount: g(250_000), date: d('2026-09-10'), status: 'CONFIRMED', recurringRuleId: 'rent' },
        ],
      }),
    );
  ...
  it('zobowiązanie tygodniowe: zostają tylko nieopłacone wystąpienia', () => {
    const gym: BudgetExpenseRule = {
      ...
      isActive: true,
      schedule: { frequency: 'WEEKLY', interval: 1, startDate: d('2026-01-01'), dayOfMonth: null, dayOfWeek: 5 },
    };
    const input = assembleBudgetInput(
      snapshot({
        expenseRules: [gym],
  ...
  it('opłata z poprzedniego okresu nie liczy się jako opłata w tym', () => {
    const input = assembleBudgetInput(
      snapshot({
        expenseRules: [rent()],
        transactions: [
          { amount: g(250_000), date: d('2026-08-10'), status: 'CONFIRMED', recurringRuleId: 'rent' },
        ],
      }),
    );
    expect(input.remainingFixedCommitments).toEqual([{ label: 'Czynsz', amount: 250_000 }]);
...
describe('assembleBudgetInput — cele', () => {
    ...
    const input = assembleBudgetInput(
      snapshot({
        goals: [
          ...
          { id: 'trip', targetAmount: g(100_000), currentAmount: g(0), targetDate: d('2026-11-30') },
        ],
      }),
    );
  ...
  it('termin celu w przeszłości → cała brakująca kwota w tym okresie', () => {
    const input = assembleBudgetInput(
      snapshot({
        goals: [
          { id: 'late', targetAmount: g(100_000), currentAmount: g(40_000), targetDate: d('2026-06-01') },
        ],
      }),
    );
  ...
  it('osiągnięty cel → rata 0', () => {
    const input = assembleBudgetInput(
      snapshot({
        goals: [
          { id: 'done', targetAmount: g(100_000), currentAmount: g(100_000), targetDate: d('2027-01-01') },
        ],
      }),
    );
