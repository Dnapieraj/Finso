...
describe("createIncomeSourceSchema", () => {
  it("REGULAR wymaga oczekiwanej kwoty", () => {
    expect(createIncomeSourceSchema.safeParse({ name: "Pensja", kind: "REGULAR" }).success).toBe(
      false,
    );
    expect(
  ...
  it("IRREGULAR nie może mieć oczekiwanej kwoty — prognozę liczy silnik", () => {
    ...
    expect(
      createIncomeSourceSchema.safeParse({ name: "Zlecenia", kind: "IRREGULAR" }).success,
    ).toBe(true);
  });

  it("incomeSourceShapeSchema sprawdza stan po scaleniu PATCH", () => {
    expect(
      incomeSourceShapeSchema.safeParse({ kind: "IRREGULAR", expectedAmount: 500 }).success,
    ).toBe(false);
  });
});
...
describe("domyślne wartości tylko przy tworzeniu", () => {
  it("nowy wpływ i wydatek są domyślnie CONFIRMED", () => {
    ...
    expect(createTransactionSchema.parse({ amount: 100, date: "2026-09-10" }).status).toBe(
      "CONFIRMED",
    );
  });
